...
class Pendulum {
    ...
    toggle() {
        if (this.interval === null) {
            ...
            document.querySelector("#toggle").textContent = "Остановить";
        } else {
            clearInterval(this.interval);
            this.interval = null;
            document.querySelector("#toggle").textContent = "Запуск";
        }
    }
}
...
document.querySelector("#length1").addEventListener("input", () => {
    ...
    document.querySelector("#length1-label").textContent = "Длина 1 нити: " + document.querySelector("#length1").value + "м";
    pendulum.draw();
});
document.querySelector("#length2").addEventListener("input", () => {
    pendulum.l2 = parseInt(document.querySelector("#length2").value);
    document.querySelector("#length2-label").textContent = "Длина 2 нити: " + document.querySelector("#length2").value + "м";
    pendulum.draw();
});
...
document.querySelector("#mass1").addEventListener("input", () => {
    ...
    document.querySelector("#mass1-label").textContent = "Масса 1 шара: " + document.querySelector("#mass1").value + "кг";
    pendulum.draw();
});
document.querySelector("#mass2").addEventListener("input", () => {
    pendulum.m2 = parseInt(document.querySelector("#mass2").value);
    document.querySelector("#mass2-label").textContent = "Масса 2 шара: " + document.querySelector("#mass2").value + "кг";
    pendulum.draw()
});
pendulum.draw();
